"""
Ollama Langflow Component with Nginx Basic Auth Support

วิธีใช้งานร่วมกับ Nginx Basic Authentication ผ่านหน้าจอ Langflow UI:
-------------------------------------------------------------------------
ในช่อง 'Ollama API Key / Auth Basic' คุณสามารถกรอกข้อมูลได้ 2 รูปแบบ:

รูปแบบที่ 1: กรอกแบบ Username และ Password ตรงๆ (แนะนำ - สะดวกที่สุด)
    - ใส่ค่าใน Format -> username:password
    - ตัวอย่าง: admin:MySecurePassword123
    - คอมโพเนนต์จะนำไปเข้ารหัส Base64 และเติมโครงสร้าง HTTP Basic Auth ให้โดยอัตโนมัติ

รูปแบบที่ 2: กรอกแบบพรี-เอนโค้ด (กรณีแปลงรหัสผ่านเป็น Base64 มาจากข้างนอกแล้ว)
    - ใส่ค่าใน Format -> Basic <ตามด้วยรหัส Base64>
    - ตัวอย่าง: Basic YWRtaW46TXlTZWN1cmVQYXNzd29yZDEyMw==

* หมายเหตุ: หากข้อความในช่องไม่มีเครื่องหมาย ":" และไม่มีคำว่า "Basic " 
           ระบบจะส่งข้อมูลแบบ "Bearer Token" ตามมาตรฐานเดิมของคอมโพเนนต์
-------------------------------------------------------------------------
"""
...
class ChatOllamaComponent(LCModelComponent):
    display_name = "CHN: Ollama (Basic Authentication)"
    description = "Generate text using Ollama Local LLMs with Basic Authentication."
    icon = "Ollama"
    name = "OllamaModel"
    ...
    inputs = [
        ...
        SecretStrInput(
            name="api_key",
            display_name="Ollama API Key / Auth Basic",
            info="Your Ollama API key, Nginx Basic Auth (username:password), or 'Basic <base64>'.",
            value=None,
            required=False,
            real_time_refresh=True,
            advanced=True,
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                for model in models[self.JSON_MODELS_KEY]:
                    ...
                    # If capabilities not provided, assume it's a completion model
    ...
    def headers(self) -> dict[str, str] | None:
        """Get the headers for the Ollama API, supporting both Bearer and Nginx Basic Auth."""
        if self.api_key and self.api_key.strip():
            api_key_str = self.api_key.strip()
            
            # 1. หากผู้ใช้ใส่โครงสร้าง "Basic <base64>" มาเองโดยตรง
            if api_key_str.startswith("Basic "):
                return {"Authorization": api_key_str}
                
            # 2. หากกรอกแบบ "username:password" ให้เข้ารหัส Base64 อัตโนมัติ
            elif ":" in api_key_str and not api_key_str.startswith("http"):
                encoded = base64.b64encode(api_key_str.encode("utf-8")).decode("utf-8")
                return {"Authorization": f"Basic {encoded}"}
            
            # 3. นอกเหนือจากนั้นให้ส่งเป็น Bearer token ตามปกติ
