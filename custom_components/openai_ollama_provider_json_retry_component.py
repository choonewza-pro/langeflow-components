...
class NTMatchaJSONRetryComponent(Component):
    display_name = "NT MATCHA (JSON + Retry)"
    description = (
        "เรียก NT MATCHA (LLM) ให้ตอบเป็น JSON แล้ว parse ทันที ถ้า parse ไม่สำเร็จ "
        "จะเรียกซ้ำอัตโนมัติสูงสุดตามจำนวนที่กำหนด ถ้าครบจำนวนแล้วยังไม่ผ่าน จะส่งออกทาง Error"
    )
    icon = "Repeat"
    name = "NTMatchaJSONRetry"
    ...
    inputs = [
        ...
        DropdownInput(
            ...
            value="OpenAI",
            info="เลือก provider ของ LLM ที่จะเรียก",
        ...
        StrInput(
            ...
            value="gpt-5-mini",
            required=True,
            info="OpenAI: ชื่อ model เช่น gpt-5-mini | Ollama: ชื่อ model ที่ pull ไว้ เช่น llama3.1, qwen2.5vl",
        ),
        StrInput(
            name="openai_api_base",
            display_name="OpenAI API Base",
            info="Base URL ของ NT MATCHA / AI Gateway (เฉพาะ provider = OpenAI)",
            show=True,
        ),
        SecretStrInput(
            name="api_key",
        ...
        StrInput(
            ...
            info="Base URL ของ Ollama server (เฉพาะ provider = Ollama)",
            show=False,
        ),
        DictInput(
            name="ollama_headers",
            display_name="Ollama HTTP Headers",
            info="HTTP Header เพิ่มเติมสำหรับเรียก Ollama server (เช่น Authorization ถ้าอยู่หลัง reverse proxy) "
            "ใส่ได้หลายคู่ key-value ตามต้องการ (เฉพาะ provider = Ollama)",
            advanced=True,
            show=False,
        ),
        SliderInput(
    ...
    def _build_llm(self) -> Any:
        if self.provider == "Ollama":
            ...
            if self.ollama_headers:
                client_kwargs["headers"] = self.ollama_headers
            ...
            return ChatOllama(
                ...
                format="json",  # บังคับให้ Ollama ตอบเป็น JSON เหมือนฝั่ง OpenAI
                client_kwargs=client_kwargs or None,
            )

        # default: OpenAI (รวมถึง NT MATCHA / OpenAI-compatible gateway)
        api_key_value = None
        if self.api_key:
            api_key_value = (
    ...
    def update_build_config(self, build_config: dict, field_value: Any, field_name: str | None = None) -> dict:
        if field_name == "provider":
            is_openai = field_value == "OpenAI"
            if "api_key" in build_config:
                build_config["api_key"]["show"] = is_openai
                build_config["api_key"]["required"] = is_openai
            if "openai_api_base" in build_config:
                build_config["openai_api_base"]["show"] = is_openai
            if "ollama_base_url" in build_config:
                build_config["ollama_base_url"]["show"] = not is_openai
            if "ollama_headers" in build_config:
                build_config["ollama_headers"]["show"] = not is_openai
            if "frequency_penalty" in build_config:
                build_config["frequency_penalty"]["show"] = is_openai
            if "presence_penalty" in build_config:
                build_config["presence_penalty"]["show"] = is_openai
        return build_config
    ...
    def _build_messages(self) -> list[Any]:
        ...
        ให้ Langflow resolve/encode รูปภาพให้ (รองรับทั้ง local storage และ S3) แทนที่จะ
        เขียน base64/path-resolve เอง ซึ่งเป็นจุดที่พังก่อนหน้านี้
        """
        messages: list[Any] = []
    ...
    def _call_llm_with_retry(self) -> str:
        ...
        คืนค่า text คำตอบดิบจาก LLM (ไม่แปลงเป็น dict/object) เมื่อสำเร็จ
        เก็บ log การ retry ไว้ใน self.ctx เพื่อ debug ย้อนหลังได้
        """
        ...
        for attempt in range(1, max_retries + 1):
            ...
            except Exception as e:  # noqa: BLE001 - ครอบคลุม timeout / connection / API error ทั้งหมด
                last_error = e
                attempts_log.append(
                    {
                        "attempt": attempt,
    ...
    def _get_result(self) -> tuple[str | None, str | None]:
        ...
        cache ผลลัพธ์ไว้ใน self.ctx เพื่อไม่ให้เรียก LLM ซ้ำ ในกรณีที่ทั้ง Success และ Error
        ...
        # ตรวจสอบรูปภาพก่อนเรียก LLM เลย (เฉพาะตอน require_image เปิดอยู่) — ถ้าไม่มีรูป ให้ error ทันที ไม่ retry ไม่ยิง LLM
